    ...
    <div class="flex flex-col lg:flex-row mt-8 justify-between relative z-10">
        <div class="flex flex-grow flex-col rounded-lg overflow-hidden lg:max-w-640px -ml-2 sm:-ml-4 lg:ml-0 w-contact-mobile sm:w-contact lg:w-3/5 bg-white shadow-xl pb-6">
            ...
            <div class="px-6 flex flex-col text-lg text-gray-blue f">
                <div class="flex w-full pt-5 pb-2 border-b border-gray-border relative">
                    <label class="font-semibold md:text-lg text-sm" for="email">From</label>
                    <input v-model="email" class="pl-3 bg-none outline-none md:text-lg text-sm flex-grow" id="email" placeholder="example@domain.com" />
                ...
                <div class="w-full flex flex-row pt-5 pb-2 border-b items-center border-gray-border">
                    ...
                    <div class="pl-3 flex flex-row">
                        <div class="bg-light rounded-full flex flex-row items-center pl-1 pr-3 py-1">
                            <a tabindex="-1" class="pl-2 select-none md:text-lg font-medium text-sm" href="mailto:snoohd@gmail.com">snoohd@gmail.com</a>
                        </div>
                    </div>
                </div>
                <div class="w-full pt-5 pb-2 border-b border-gray-border flex">
                    <label class="font-semibold md:text-lg text-sm" for="subject">Subject</label>
                    <input v-model="subject" class="pl-3 bg-none outline-none md:text-lg text-sm flex-grow" placeholder="New job offer" id="subject"/>
                </div>
                <div class="w-full py-5 relative">
                    <textarea v-model="message" id="message" class="w-full resize-none lg:h-150px md:h-240px h-150px xl:h-240px bg-none font-normal outline-none md:text-lg text-sm" placeholder="Message" />
                ...
                <div class="w-full flex flex-row justify-end">
                    <button @click="sendEmail"
                        ...
                        <span v-if="sendError === 'server'" class="shake text-white md:text-lg font-bold py-1 text-base">Server error!</span>
                        <span v-else-if="sendError === 'again'" class="text-white md:text-lg font-bold py-1 text-base">Try again</span>
                        <span v-else-if="sending === 'stage'" class="text-white md:text-lg font-bold py-1 text-base">Sending...</span>
                        <span v-else-if="send" class="text-white md:text-lg font-bold py-1 text-base">Sent!</span>
                        <span v-else-if="oops" class="text-white md:text-lg font-bold py-1 text-base">Oops!</span>
                        <span v-else class="text-white md:text-lg font-bold py-1 text-base">Send</span>
                        <svg
                            :class="[
        ...
        <div class="flex flex-col justify-center flex-grow-0">
            <ul class="lg:pl-24 mt-16 lg:mt-0 pl-0 sm:w-full justify-between md:w-3/4 lg:w-full flex flex-col xs:flex-row xs:flex-wrap lg:block font-normal items-start text-white lg:text-gray sm:text-base md:text-lg space-y-8 xs:space-y-0 lg:space-y-12">
...
export default {
    data(){
        return {
            ...
            contactInfo: [{
                content: '(+31) 631 979 501',
                link: 'tel:+31631979501',
                icon: 'phone',
                size: '95%',
                color: '#43bb5a'
            },{
