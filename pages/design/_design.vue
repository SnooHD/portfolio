...
<div class="flex flex-col justify-center items-center" v-if="param">
    <div
        ...
        "
    >
        <sectionTitle type="h1" :class="{'s_in-view' : isMounted }" :title="design.title" />
        <div class="mt-4 md:mt-6 lg:mt-8 font-normal text-lg leading-wider" v-html="design.content"></div>
    </div>
    <div 
        ...
        :style="{
            ...
            boxShadow: `0px ${shadowTop}px ${shadowBlur}px -12px rgba(0,0,0,${alpha || '0'})`
        }">
        <picture>
            <source
                v-for="src in ['png', 'webp']"
                :key="`design-image-${index}-src-${src}`"
                class="w-full"
                :srcset="`/images/design/${param}-full.${src}`"
                :type="src === 'webp' ? 'image/webp' : 'image/png'"
            />
            <img
                class="w-full"
                :src="`/images/design/${param}-full.png`"
            />
        </picture>
    </div>
        ...
        <div>
            <a href="../../#design">
                <button class="
                    ...
                    text-sm px-6 py-2
                ">
                    <svg class="group-hover:-translate-x-2px transition-400 transition-transform" width="18" viewBox="0 5 100 100" fill="#fff" xmlns="http://www.w3.org/2000/svg">
