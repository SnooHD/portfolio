<template>
  ...
  >
    <div
      class="flex flex-col justify-center items-start relative z-10 w-full flex-shrink-0 sm:w-280px lg:w-363px"
    >
      ...
      >
        <div
          ...
          :style="
            `transform: translateY(${translateTriangle}px) translateX(-1px)`
          "
        ></div>
        <template v-for="(item, index) in items">
          ...
          >
            <div
              class="flex flex-shrink-0 transition-opacity transition-400 items-center hover:opacity-100"
              :class="activeItem === index ? 'opacity-100' : 'opacity-60'"
            >
              <icons
                :icon="item.icon"
                :class="item.color"
    ...
    <div
      class="flex w-full relative sm:static lg:relative justify-center mt-8 sm:mt-0 lg:pt-7 flex flex-col items-end flex-grow  sm:max-w-267px md:max-w-616px"
    >
      <blob
        class="absolute w-full top-.5 sm:top-.15 md:top-0 transform-dev-mobile-blob sm:transform-dev-blob -right-1 lg:right-0"
        :overlay="items[activeItem].overlay"
        :item="activeItem"
      />
      <div
        class="relative flex w-full sm:max-w-267px lg:max-w-616px sm:transform-mobile-top lg:transform-none items-center"
      >
        <div
          @click="setImage('previous')"
          class="sm:hidden -ml-15px xs:ml-0 shadow-lg active:shadow flex items-center justify-center bg-white flex-shrink-0 rounded-full w-14 h-14 xs:w-16 xs:h-16"
        >
          <i class="rotate-180 text-gray">
            ...
            >
              <path
                d="M0 0.450014V7.13001C0 7.50001 0.42 7.71001 0.72 7.49001L5.58 3.85001C5.83 3.66001 5.81 3.28001 5.55 3.11001L0.68 0.0700138C0.39 -0.119986 0 0.100014 0 0.450014Z"
              />
            </svg>
          </i>
        </div>
        ...
        <carousel
          ...
          :type="type"
          :items.sync="carouselItems"
          :active="activeItem"
        />

        <div
          @click="setImage('next')"
          class="sm:hidden -mr-15px xs:mr-0 bg-white shadow-lg active:shadow flex items-center justify-center flex-shrink-0 rounded-full w-14 h-14 xs:w-16 xs:h-16"
        >
          <i class="text-gray">
            <svg
              width="12px"
    ...
    carouselItems: {
      get() {
        return this.items.filter(
          item => typeof item[this.type] === "undefined" || item[this.type]
        );
      },
      set(items) {
      ...
      items: [
        {
          ...
          buttonText: "Showcase",
          content:
            'Showcase for the <b class="font-medium">React</b> made Basic-Fit kiosk app.'
        },
        {
          name: "Custom Fotowand",
          webp: "fotowand.webp",
          jpg: "fotowand.jpeg",
          url: "https://customfotowand.nl/zelf-ontwerpen/",
          icon: "vue",
          overlay: "28a254",
          buttonText: "Website",
          content:
            'A <b class="font-medium">Vue</b> component for desiging your own custom wallpaper.'
        },
        {
          name: "Nativeway",
          webp: "nativeway.webp",
          ...
          buttonText: "Website",
          content:
            'Website for an English Academy in Montevideo, made with <b class="font-medium">Nuxt</b>'
        }
        // {
